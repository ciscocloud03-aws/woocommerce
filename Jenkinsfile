             ...
             steps {
                 ...
                 script {
                     withCredentials([usernamePassword(credentialsId: 'github_pw', passwordVariable:"password", usernameVariable: "username")]) {
                         ...
                         sh "git init && sed -i 's@image: .*@image: 339712790288.dkr.ecr.ap-northeast-2.amazonaws.com/woocommerce:${env.BUILD_NUMBER}@g' /var/jenkins_home/workspace/woocommerce/woo-manifest/woo-deploy.yaml"
                         sh "cd /var/jenkins_home/workspace/woocommerce/woo-manifest && cat woo-deploy.yaml && id && git add -A"
                         sh "git status" // 상태 확인
